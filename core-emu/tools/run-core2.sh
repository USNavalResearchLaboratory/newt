...
python_file=$4
args=$5
args=${@:6} # Need to grab rest of args ...
...
if [[ ! -n "$imnfile" && ! -n "$python_file" ]] ; then
    echo "Usage: ./run-core2.sh file.imn python_class_name [args] [browser] [resize]"
    exit 1
fi
...
do
    ...
    vcmd -c $node_dir -- $root/bin/core.sh $PYTHON_FILE_NAME -m core $args &
done < "/tmp/nodes.txt"
